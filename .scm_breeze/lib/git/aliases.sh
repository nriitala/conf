...
if [ "$git_setup_aliases" = "yes" ]; then
  ...
  __git_alias "$git_commit_alias"      "git" "commit" "-v"
  ...
  _alias $git_commit_amend_alias='git commit --amend -v'
  # Add staged changes to latest commit without prompting for message
  _alias $git_commit_amend_no_msg_alias='git commit -v --amend -C HEAD'
  _alias $git_commit_no_msg_alias='git commit -v -C HEAD'
